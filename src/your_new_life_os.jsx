...
import {
 ...
 Pencil, ArrowLeft, GripVertical, NotebookPen, LogOut, Loader2,
} from 'lucide-react';
import { useAuth } from './AuthContext';
import {
 loadCloudData, saveCloudData, subscribeToCloudData,
...
/* =========================================================================
  5a. PERSISTENCE
  =========================================================================
  Persistence has moved to ./storage.js — a two-tier system:
   · loadLocalCache / saveLocalCache  → instant per-user localStorage cache
   · loadCloudData / saveCloudData    → Supabase canonical store
   · subscribeToCloudData             → real-time updates from other devices

  The AppProvider below wires these into a single source of truth: writes
  go to both layers; the cache primes immediately on load while cloud data
  reconciles in the background; a subscription pushes remote changes back.  */
...
function AppProvider({ children }) {
 ...
 // Try the local cache first — paints instantly while we await the cloud.
 // If the cache is empty (first run on this device for this user), seed
 // structural data so the user has something to interact with on day one.
 const cached = userId ? loadLocalCache(userId) : null;
 const hasSeed = (s) => Array.isArray(s) && s.length > 0;

 const [aspects, setAspects] = useState(hasSeed(cached?.aspects) ? cached.aspects : SEED_ASPECTS);
 const [kpis, setKpis]    = useState(hasSeed(cached?.kpis)    ? cached.kpis    : SEED_KPIS);
 const [entries, setEntries] = useState(cached?.entries ?? []);
 const [notes, setNotes]   = useState(cached?.notes ?? {});

 // Sync status surfaced to the UI so the user knows their data is safe.
 const [syncState, setSyncState] = useState('idle'); // 'idle' | 'syncing' | 'saved' | 'offline'
 const [bootLoaded, setBootLoaded] = useState(false);

 // Refs used to (a) debounce frequent writes, (b) detect self-writes coming
 // back through the real-time subscription so we don't loop forever.
 const saveTimer = useRef(null);
 const lastSavedJSON = useRef('');
 const skipNextWrite = useRef(false); // set when we apply an incoming remote update

 // 1. Boot: load canonical data from the cloud.
 useEffect(() => {
  if (!userId) return;
  let cancelled = false;
  setSyncState('syncing');
  loadCloudData(userId).then((cloud) => {
   if (cancelled) return;
   // Migrate any KPIs missing criticality (forward-compat with older saves).
   const migratedKPIs = (cloud.kpis || []).map((k) => ({
    ...k, criticality: normalizeCriticality(k.criticality),
   }));
   // First-time users get the seed scaffold; returning users get their data.
   const isFresh = (!cloud.aspects || cloud.aspects.length === 0)
          && (!cloud.kpis || cloud.kpis.length === 0)
          && (!cloud.entries || cloud.entries.length === 0);
   if (isFresh) {
    setAspects(SEED_ASPECTS);
    setKpis(SEED_KPIS);
    setEntries([]);
    setNotes({});
   } else {
    setAspects(cloud.aspects);
    setKpis(migratedKPIs);
    setEntries(cloud.entries);
    setNotes(cloud.notes);
   }
   lastSavedJSON.current = JSON.stringify({
    aspects: isFresh ? SEED_ASPECTS : cloud.aspects,
    kpis:    isFresh ? SEED_KPIS    : migratedKPIs,
    entries: isFresh ? []           : cloud.entries,
    notes:   isFresh ? {}           : cloud.notes,
   });
   setBootLoaded(true);
   setSyncState('saved');
  }).catch(() => {
 ...
 // 2. Save-on-change: debounced cloud upsert + immediate local cache.
 // Skip on first paint, on the tick after a remote update, and when the
 // serialized snapshot is identical to what we already saved.
 useEffect(() => {
  if (!userId || !bootLoaded) return;
  if (skipNextWrite.current) {
   skipNextWrite.current = false;
   return;
  }
  const snapshot = { aspects, kpis, entries, notes };
  const json = JSON.stringify(snapshot);
  if (json === lastSavedJSON.current) return;

  // Local cache is synchronous and fast — write it immediately.
  saveLocalCache(userId, snapshot);

  // Cloud write: debounce so a flurry of toggles becomes one network call.
  if (saveTimer.current) clearTimeout(saveTimer.current);
  setSyncState('syncing');
  saveTimer.current = setTimeout(async () => {
   await saveCloudData(userId, snapshot);
   lastSavedJSON.current = json;
   setSyncState('saved');
  }, 400);
 }, [aspects, kpis, entries, notes, userId, bootLoaded]);

 // 3. Real-time subscription: when another device modifies our row, apply
 // it locally — but only if it's actually different from what we have.
 useEffect(() => {
  if (!userId) return;
  const unsubscribe = subscribeToCloudData(userId, (incoming) => {
   const json = JSON.stringify(incoming);
   if (json === lastSavedJSON.current) return; // echo of our own write
   lastSavedJSON.current = json;
   skipNextWrite.current = true; // don't re-save what we just received
   setAspects(incoming.aspects);
   setKpis(incoming.kpis.map((k) => ({
    ...k, criticality: normalizeCriticality(k.criticality),
   })));
   setEntries(incoming.entries);
...
function TopBanner() {
 ...
 return (
  ...
  >
   ...
   <div className="container-xl mx-auto px-3 sm:px-6 py-3 sm:py-5 flex items-center justify-between gap-3 sm:gap-6">
    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
     ...
     <div className="min-w-0">
      <h1 className="font-display text-2xl sm:text-3xl md:text-4xl tracking-tight text-neutral-50 leading-none">
       Your New Life<span className="text-rose-600">.</span>
      </h1>
      <p className="label-mini text-neutral-500 mt-1 font-mono-ui hidden sm:block">
       Self-Development OS
      </p>
     </div>
    </div>
...
function Header() {
 const {
  searchQuery, setSearchQuery, setSidebarOpen,
  currentPage, syncState, user,
 } = useApp();
 const { signOut } = useAuth();
 const [searchOpen, setSearchOpen] = useState(false); // mobile expand toggle
 const searchInputRef = useRef(null);

 // When the user taps the icon on mobile, focus the input as soon as it mounts.
 useEffect(() => {
 ...
 const closeMobileSearch = () => {
  setSearchOpen(false);
  // Don't clear the query — the user may want to keep it active.
 };

 // Sync indicator copy + color
 const syncMeta = (() => {
  switch (syncState) {
   case 'syncing': return { dot: 'bg-amber-500',  text: 'SYNCING'  };
   case 'saved':   return { dot: 'bg-emerald-500', text: 'SYNCED'   };
   case 'offline': return { dot: 'bg-neutral-500', text: 'OFFLINE'  };
   default:        return { dot: 'bg-rose-600 animate-pulse', text: currentPage === 'home' ? 'DASHBOARD' : 'MANAGEMENT' };
  }
 })();
 ...
 return (
  <div className="sticky sticky-under-banner z-30 border-b border-neutral-800 backdrop-blur-xl" style={{ background: 'rgba(8,8,10,0.8)' }}>
   <div className="container-xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex items-center gap-2 sm:gap-4">
    <button
     onClick={() => setSidebarOpen(true)}
     className="lg:hidden p-2 text-neutral-300 hover:bg-neutral-800/60 rounded-md shrink-0"
     aria-label="Menu"
    >
     <Menu size={18} />
    </button>

    {/* Desktop: always-visible search input. Mobile: collapses to an icon. */}
    ...
    {/* Mobile: search toggle. When opened, replaces other header content. */}
     ...
     {searchOpen ? (
      <div className="relative flex-1">
       ...
       <input
        ...
        placeholder="Search…"
        className="w-full bg-neutral-900 border border-neutral-800 rounded-md pl-9 pr-9 py-2 text-sm text-neutral-100 placeholder-neutral-500 focus:outline-none focus:border-rose-600/50 focus:ring-1 focus:ring-rose-600/30"
       />
       <button
        onClick={() => { setSearchQuery(''); closeMobileSearch(); }}
      ...
      <button
       ...
       className="p-2 text-neutral-300 hover:bg-neutral-800/60 rounded-md"
    ...
    {/* Right-side cluster: sync indicator + user menu. Hidden when mobile
      search is expanded so the input gets the full width. */}
    {!searchOpen && (
     <>
      <div className="hidden sm:flex items-center gap-2 text-xs text-neutral-500 font-mono-ui shrink-0">
       <span className={`h-1.5 w-1.5 rounded-full ${syncMeta.dot}`} />
       {syncMeta.text}
      </div>
      <button
       onClick={signOut}
       className="p-2 text-neutral-500 hover:text-rose-400 hover:bg-neutral-800/60 rounded-md shrink-0"
...
function HomePage() {
 ...
 return (
  <div className="container-xl mx-auto px-3 sm:px-6 py-5 sm:py-8">
   {/* Page header */}
   <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
    <div>
     ...
     <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-neutral-50 tracking-tight">
      {aspectFilter
...
function RadarSection({ aspects }) {
 ...
 // Custom clickable tick for angle labels — wraps long names onto a second
 // line, truncates extreme cases with an ellipsis, and exposes the full
 // original text through a native <title> tooltip for hover fallback.
 //
 // Wrapping strategy:
 //  · <= 15 chars     → single line as-is
 //  · 16–30 chars, has space → greedy two-line split at word boundary
 //  · single word > 15       → truncate to 14 chars + "…"
 //  · still > 2 lines        → keep first line, merge rest into truncated second
 const wrapLabel = (text, maxLineLen = 15) => {
  const s = String(text || '');
  if (s.length <= maxLineLen) return [s];
  const words = s.split(/\s+/);
  if (words.length === 1) {
   return [s.slice(0, maxLineLen - 1) + '…'];
  }
  const lines = [];
  let current = '';
  ...
  if (lines.length > 2) {
   ...
   const truncated = rest.length > maxLineLen ? rest.slice(0, maxLineLen - 1) + '…' : rest;
   return [first, truncated];
  }
  return lines.map((l) => l.length > maxLineLen + 2 ? l.slice(0, maxLineLen - 1) + '…' : l);
 };

 const ClickableTick = (props) => {
  const { payload, x, y, textAnchor } = props;
  const fullText = payload?.value ?? '';
  const datum = radarData.find((d) => d.aspect === fullText);
  const isLevelOne = !drillAspect;
  const lines = wrapLabel(fullText, 15);
  // For multi-line text, pull the whole block up so it stays visually
  // centered on the computed anchor point instead of drifting downward.
  const firstDy = lines.length > 1 ? `${-0.4 * (lines.length - 1)}em` : '0em';
  ...
  return (
   ...
   >
    ...
    {/* Native SVG tooltip — always shows the full untruncated label on hover */}
 ...
 return (
  <div className="bg-neutral-900/40 border border-neutral-800 rounded-lg p-4 sm:p-6 h-full relative overflow-hidden">
   {/* subtle radial glow */}
   <div
    ...
    <div className="relative radar-h">
     {radarData.length === 0 ? (
      ...
      <ResponsiveContainer width="100%" height="100%">
       <RadarChart data={radarData} outerRadius="68%" margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
...
function MatrixSection({ kpis, aspects }) {
 ...
 // Per-aspect collapse state. Default all aspects collapsed — the header
 // rows show aggregate progress per period, and expanding reveals KPIs.
 const [expandedAspects, setExpandedAspects] = useState(new Set());
 const toggleAspectExpanded = (aspectId) => {
 ...
 return (
  <div className="bg-neutral-900/40 border border-neutral-800 rounded-lg h-full flex flex-col">
   <div className="flex flex-wrap items-center justify-between gap-3 p-4 sm:p-6 border-b border-neutral-800">
    <div>
     <div className="label-mini text-neutral-500 font-mono-ui">
    ...
    <div ref={scrollRef} className="flex-1 overflow-x-auto">
     <div className="min-w-max">
      ...
      {kpisByAspect.map(({ aspect, kpis: aspectKPIs }, i) => {
       ...
       const aspectColor = getAspectColor(aspect, i);
       return (
       <div
        key={aspect.id}
        className={`transition-opacity ${isAspectDragging ? 'opacity-40' : ''}`}
       >
        {/* aspect group header — grip handle + accordion toggle + per-period aggregates */}
        <div
         className="flex bg-neutral-900/50 border-b border-neutral-800"
         onDragOver={handleAspectDragOver(aspect.id)}
         onDrop={handleAspectDrop(aspect.id)}
        >
         {/* Sticky-left cell: grip (for drag) + caret + dot + name (click toggles accordion) */}
         <div className="w-64 h-12 shrink-0 sticky left-0 bg-neutral-900/90 flex items-center border-r border-neutral-800 select-none">
          <div
           draggable
           onDragStart={handleAspectDragStart(aspect.id)}
           onDragEnd={handleDragEnd}
           className="pl-2 pr-1 h-full flex items-center cursor-grab active:cursor-grabbing text-neutral-600 hover:text-neutral-400 shrink-0"
          ...
          <button
           ...
           className="flex-1 h-full flex items-center gap-2 pl-1 pr-3 text-left hover:bg-neutral-800/40 transition-colors min-w-0"
           aria-expanded={isExpanded}
           aria-label={`${isExpanded ? 'Collapse' : 'Expand'} ${aspect.name}`}
          >
           {isExpanded
           ...
           />
           <span className="label-micro font-mono-ui text-neutral-300 truncate">
            {aspect.name}
           </span>
           <span className="label-micro font-mono-ui text-neutral-600 shrink-0 ml-auto">
            {aspectKPIs.length}
           </span>
          </button>
         </div>
         {/* Per-period aggregate cells: successes / total for this aspect × period.
           Columns stay width-locked (w-16) so alignment is preserved whether or
           not the aspect is expanded — the scroll position is the outer container's,
           shared with the KPI rows and the header. */}
         {periodIds.map((pid) => {
          const total = aspectKPIs.length;
          const successes = aspectKPIs.reduce((n, k) => {
           const entry = entries.find((e) => e.kpiId === k.id && e.periodId === pid);
           return entry?.value === 1 ? n + 1 : n;
          }, 0);
          const isCurrent = pid === currentPeriod;
          const isFuture = isFuturePeriod(pid, periodType);
          ...
          return (
           <div
            key={pid}
            className={`w-16 h-12 shrink-0 relative flex items-center justify-center border-r border-neutral-800/30
             ${isCurrent ? 'bg-rose-600/5' : ''}
             ${isFuture && !isCurrent ? 'bg-neutral-900/30' : ''}`}
            title={`${successes}/${total} in ${formatPeriodLabel(pid, periodType)}`}
           >
            {/* subtle fill bar at bottom indicating success ratio */}
            {total > 0 && (
             <div
              className="absolute bottom-0 left-0 right-0"
              style={{
               height: `${Math.max(2, ratio * 28)}px`,
               background: ratio > 0
                ? (isFuture ? `${aspectColor}44` : `${aspectColor}55`)
                : 'transparent',
              }}
             />
            )}
            <span
             className={`relative label-micro font-mono-ui
              ${total === 0 ? 'text-neutral-700'
               : ratio === 1 ? 'text-neutral-100'
               : ratio > 0 ? 'text-neutral-300'
               : 'text-neutral-600'}`}
            >
             {total > 0 ? `${successes}/${total}` : '—'}
            </span>
           </div>
          );
         })}
        ...
        {/* KPI rows — only rendered when this aspect is expanded */}
        {isExpanded && aspectKPIs.map((kpi) => {
         ...
         return (
          ...
          >
           <div
            ...
            title={`Drag to reorder within ${aspect.name}`}
           >
...
function HistoricalSection({ aspects }) {
 ...
 return (
  <div className="bg-neutral-900/40 border border-neutral-800 rounded-lg p-4 sm:p-6">
...
function NotepadSection({ aspects }) {
 ...
 return (
  <div className="bg-neutral-900/40 border border-neutral-800 rounded-lg flex flex-col h-full">
   <div className="flex items-center justify-between p-4 sm:p-6 border-b border-neutral-800 gap-3">
    <div className="min-w-0">
...
function AppShell() {
 const { currentPage, bootLoaded } = useApp();
 return (
  <>
   ...
   <style>{`
    ...
    body {
     ...
     /* iOS niceties: prevent text-size autoinflate, allow safe-area padding */
     -webkit-text-size-adjust: 100%;
     -webkit-tap-highlight-color: transparent;
     overscroll-behavior-y: none;
    }
    /* Honor the iPhone notch/home-indicator on PWA installs */
    ...
    /* Layout sizing helpers — mobile-first defaults, scale up on larger viewports */
    .container-xl { max-width: 1800px; margin-inline: auto; }
    .stat-min   { min-width: 96px; }
    .radar-h    { height: 280px; }
    .line-h    { height: 240px; }
    .accent-bar  { height: 2px; }
    .sidebar-h   { height: calc(100vh - 70px); }
    .sticky-under-banner { top: 70px; }
    ...
    /* On phones, hide the scrollbar entirely — touch users get momentum scrolling */
